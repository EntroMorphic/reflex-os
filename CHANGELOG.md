...
### Added
- **Phase 33 Self-Expanding Perception** — under sustained pain (5+ consecutive ticks), the supervisor walks the shadow atlas for HARDWARE_IN register-level entries not in the live Loom, pages in up to 2 per pulse, and lets Hebbian learning + eviction decide which survive. Metabolic-gated (suspended in surviving, halved in conserving). Telemetry: `#T:D,<name>`. Shell: `status` shows exploration state. No new files — a ~50-line sub-pass of the existing supervisor.
