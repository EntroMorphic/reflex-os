...
| `kernel wdt [ms|off|observe <ms>|expired|entry]` | Read, disarm, or arm the low-power watchdog (admin). Arming needs a build flag, because a fired watchdog does not return a usable board. `observe` arms it to latch instead of reset, which is safe; `expired` reads that latch; `entry` reports the watchdog state this boot started with, captured before startup disarmed it |
