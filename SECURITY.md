...
-   **Key provisioning:** on first boot (or NVS wipe), a unique 16-byte key is generated via `reflex_hal_random_fill()` and persisted to NVS under `goose/aura_key`. That is `esp_fill_random()` on the classic ESP32, but on the C6 — the primary target — it reads `RNG_DATA_REG` directly, XORing two reads per word. A latched register (a noise source that has not started) collapses that XOR to zero for every word, so the generated key is now health-checked for degenerate output before being persisted, retried, and failing that, replaced by the MAC-derived fallback below rather than a key of all zeros that every board in the same state would also hold. Two factory-fresh boards therefore do not accidentally trust each other. Pairing requires an operator to run `aura setkey <hex>` on both peers with a chosen shared key. A compile-time default is retained only as a last-resort fallback if NVS writes fail.
-   **Replay protection:** a 64-slot time-bounded replay cache in the RX path rejects packets whose `(src_mac, nonce)` pair has been seen within a 5-second window. Stale entries outside the window are treated as empty. The slot index is a hash over the nonce and the *full* sender MAC (`goose_policy_replay_slot`), mixed so that every input bit reaches the low bits the cache indexes on, so two peers with colliding nonce low bits land in different slots and don't evict each other. This claim was false until 2026-09-07: the previous expression placed the MAC bytes at bit positions 8-23 and then masked to 6 bits, so the slot was exactly `nonce & 63` for every sender and the MAC was discarded entirely. It is now pinned by a host test that asserts 64 peers sharing one nonce spread across the ring.
...
- **Discovery Throttling:** Nodes rate-limit incoming queries to 10Hz, preventing Mesh Denial-of-Service (DoS) attacks. Outbound QUERY broadcasts are gated at the same 10Hz. Egress was previously ungated, and `goonies_resolve_cell` emits a broadcast QUERY on *every* unresolved `peer.*` lookup — a name resolution reachable from the observer role, the least privileged one — so a caller asking for unresolvable peer names in a loop could drive a board as a broadcast amplifier against its own mesh. Suppressed queries are counted as `query_throttled` in `mesh stat`; a rising value means something local is doing exactly that.
- **Arc state is validated at ingress.** Every consumer treats `arc->state` as a trit — SYNC and MMIO_SYNC assign it straight into a cell's `state`, POSTURE multiplies it by the wire weight into the swarm accumulator — and nothing checked it. The bounds below are arithmetic that only holds while `|state| <= 1`: with weight capped at 4, a legitimate arc moves the accumulator by at most 4 against a threshold of 10. An arc carrying `state=99` moved it by 396, saturating instantly. Hardware-demonstrated on a guard-removed build: **one** such arc flipped a peer's posture, versus the three the hysteresis is designed to require. `atmosphere_recv_cb` now range-checks state once, after the Aura gate and before any op dispatch, so no future op can miss it. Rejected arcs increment `rx_malformed`, visible in `mesh stat` — a non-zero value means a peer holding the Aura key is emitting malformed arcs, which is a buggy or tampered peer rather than radio noise.
