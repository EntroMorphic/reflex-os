...
LEDGER = os.path.join(ROOT, "docs", "independence-dependency-map.md")
DOC_BEGIN = "<!-- independence-scope: generated by tools/check_independence.py -->"
DOC_END = "<!-- /independence-scope -->"


def measure_config(build):
    ...
    c = counts(on)
    return ({t: c.get(t, 0) for t in ("A", "B", "C", "D", "E", "F")},
            c.get("Z", 0), len(off))


def doc_table(configs):
    """The scope table, rendered from measurement.

    Generated rather than written, because this table is the first thing a
    reader sees and it had drifted to a state many commits old — Tier C at 8
    where it is 0, Tier E at 8 where it is 0, a total of 23 where it is 6, and
    an off-path count of 19 where it is 31. The document that records "prose
    drifts, a measurement CI runs does not" had drifted in exactly that way, in
    its own summary, while every number further down was current.
    """
    labels = {"A": "SoC constants, ROM entry points", "B": "Deep-sleep entry, heap reporting",
              "C": "FreeRTOS as the scheduler", "D": "Radio",
              "E": "Console RX, peripheral drivers", "F": "Build system, startup, heap, image"}
    names = [c for c, _ in configs]
    lines = [DOC_BEGIN, "", "| tier | " + " | ".join(f"`{n}`" for n in names) + " | |",
             "|---|" + "---|" * len(names) + "---|"]
    for t in ("A", "B", "C", "D", "E", "F"):
        cells = []
        for _n, (tiers, _z, _o) in configs:
            v = tiers[t]
            cells.append("**0 — clear**" if v == 0 else str(v))
        lines.append(f"| {t} | " + " | ".join(cells) + f" | {labels[t]} |")
    lines.append("| **on-path total** | " +
                 " | ".join(f"**{sum(t.values())}**" for _n, (t, _z, _o) in configs) + " | |")
    lines.append("| bedrock | " + " | ".join(str(z) for _n, (_t, z, _o) in configs) +
                 " | vendor binary; ratcheted by `make blob-check` |")
    lines.append("| off-path | " + " | ".join(str(o) for _n, (_t, _z, o) in configs) +
                 " | alternative backends; ratcheted as a total |")
    lines += ["", DOC_END]
    return "\n".join(lines)


def doc_sync(write):
    """Regenerate or verify the ledger's scope table. Returns an exit code."""
    builds = ["build_own_entry", "build_independence"]
    present = [b for b in builds if os.path.isfile(os.path.join(ROOT, b, "reflex_os.map"))
               or os.path.isdir(os.path.join(ROOT, b))]
    if len(present) != len(builds):
        missing = [b for b in builds if b not in present]
        print(f"  scope table: {', '.join(missing)} not built — not judging.")
        return 0
    configs = [(b, measure_config(b)) for b in builds]
    table = doc_table(configs)
    text = open(LEDGER, errors="replace").read()
    if DOC_BEGIN not in text or DOC_END not in text:
        print(f"FAILED: {os.path.relpath(LEDGER, ROOT)} has no generated scope block.")
        return 1
    start = text.index(DOC_BEGIN)
    end = text.index(DOC_END) + len(DOC_END)
    if text[start:end] == table:
        print("  scope table: matches the measurement.")
        return 0
    if write:
        open(LEDGER, "w").write(text[:start] + table + text[end:])
        print("  scope table: regenerated from the measurement.")
        return 0
    print("FAILED: the ledger's scope table no longer matches the measurement.")
    print("  It is the first thing a reader sees, and it drifts silently because")
    print("  nothing recomputes it. Regenerate with make independence-doc-update.")
    return 1
...
def main():
    # The scope table is its own mode: it measures several configurations, so it
    # cannot share the single-build state the rest of main() sets up.
    if "--check-doc" in sys.argv or "--update-doc" in sys.argv:
        return doc_sync(write="--update-doc" in sys.argv)
