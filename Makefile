...
# Formatting.
#
# CLANG_FORMAT is overridable so a specific binary can be named; an unpinned
# formatter lets the gate's verdict drift with the runner image. git-clang-format
# is only a driver script — every style decision comes from the binary it is
# handed via --binary — so the version rides on CLANG_FORMAT and the script
# itself may be whichever is on PATH.
#
# Install from pip, not apt: the pip clang-format package ships git-clang-format
# alongside the binary and the apt one does not, which is what CI installs.
CLANG_FORMAT ?= clang-format

# Files the style applies to. goose_shadow_atlas.c is excluded because it is
# generated by tools/goose_scraper.py from the SVD — style belongs to the
# generator, not to a hand-edit of its output.
FORMAT_FIND = find . \( -name '*.c' -o -name '*.h' \) \
	| grep -v build | grep -v esp-idf | grep -v goose_shadow_atlas

format:
	@$(FORMAT_FIND) | xargs $(CLANG_FORMAT) -i

# The gate checks the *lines* this branch has changed relative to BASE, not the
# whole tree. `make format-check-all` is the whole-tree check, and it does not
# pass today: most hand-written files predate any working gate, because this
# target used to pipe clang-format into `head` — a pipeline takes the status of
# its last command, so clang-format's exit code was discarded — and then echoed
# "passed" unconditionally. The CI job running it was green by construction and
# had never once failed, which is how the tree drifted this far.
#
# BASE resolution: on a branch, the merge-base with origin/main. When that is
# HEAD itself — working directly on main — an uncommitted tree is checked
# against HEAD (the work in progress) and a clean one against HEAD~1 (the commit
# just made), so the gate always has the current change in view.
#
# Gating changed lines holds new code to the style without a mass reformat that
# would rewrite brace style across the codebase and bury the history of files
# whose comments carry the design rationale. Line scope rather than file scope
# matters: a one-line fix in a legacy file must not demand reformatting the
# whole file as the price of the edit. The tree converges as code is rewritten.
BASE ?= origin/main
...
format-check:
	@$(FORMAT_FIND) | xargs $(CLANG_FORMAT) --dry-run --Werror
...
#
# BASE defaults to the previous commit; CI passes the merge base for a PR. When
# BASE is unset and the working tree is dirty, the base is HEAD instead, so a
# developer checking work in progress sees the lines they have actually edited
# rather than the previous commit's.
format-diff:
	...
	@base="$(BASE)"; \
	 ...
	 out=$$(git-clang-format --binary $(CLANG_FORMAT) --diff --extensions c,h "$$base" -- \
	        . ':(exclude)*goose_shadow_atlas.c' 2>&1); \
	 case "$$out" in \
	   *"did not modify any files"*|"") echo "Format: changed lines are clean ($$base)." ;; \
	   *"no modified files"*)           echo "Format: no C/H changes to check." ;; \
	   *) echo "$$out"; echo; \
	      echo "Changed lines are not formatted. Fix with:"; \
	      echo "  git-clang-format --binary $(CLANG_FORMAT) $$base"; \
	      exit 1 ;; \
	 esac
