 ...
 * @brief reflex_task.h implementation for the Reflex kernel scheduler mode.
 *
 * When CONFIG_REFLEX_KERNEL_SCHEDULER is enabled, the kernel owns
 * interrupt context switching (reflex_portasm.S) and scheduling policy
 * (reflex_freertos_compat.c). FreeRTOS remains as the task management
 * backend because ESP-IDF drivers (WiFi, USB-JTAG, ESP-NOW) create
 * internal FreeRTOS tasks that require its API.
 *
 * This file delegates task creation, delay, queues, and mutexes to
 * FreeRTOS — identical to reflex_task_esp32c6.c — while the kernel
 * overlay adds purpose-modulated priority policy on top.
 */
...
reflex_err_t reflex_queue_send(reflex_queue_handle_t qh, const void *item,
                               uint32_t timeout_ms) {
    if (!qh) return REFLEX_ERR_INVALID_ARG;
    TickType_t ticks = (timeout_ms == UINT32_MAX) ? portMAX_DELAY
                                                  : pdMS_TO_TICKS(timeout_ms);
    if (xQueueSend((QueueHandle_t)qh, item, ticks) != pdPASS)
...
reflex_err_t reflex_queue_recv(reflex_queue_handle_t qh, void *item,
    ...
    TickType_t ticks = (timeout_ms == UINT32_MAX) ? portMAX_DELAY
                                                  : pdMS_TO_TICKS(timeout_ms);
    if (xQueueReceive((QueueHandle_t)qh, item, ticks) != pdPASS)
