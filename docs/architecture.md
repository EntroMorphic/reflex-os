...
- **Full-Surface Coverage:** the SVD-generated shadow catalog (`goose_shadow_atlas.c`, 12,738 entries) covers 100% of the SVD-documented ESP32-C6 MMIO register + field surface (`tools/esp32c6.svd`), including fully expanded array registers (all DMA channels, LEDC channels, GPIO pins, etc.). Every entry is resolvable by name via `goose_shadow_resolve`, and the shell's `goonies find` falls through from the live registry to the catalog transparently. The `atlas verify` shell command walks the entire catalog with full round-trip equality (name, addr, mask, type, coord) plus an adjacent-pair duplicate sweep. Not included: undocumented registers, eFuse bits outside the SVD field schema, or silicon-revision deltas the SVD file doesn't capture.
