...
- **`bonsai exp5` counts 2 of the 10 pulses it sends.** The experiment transmits ten 1 ms RMT pulses into PCNT over a GPIO 4 loopback and reads the counter back; it consistently reports `overlap=2 (expected 10)` on the C6. The obvious explanation — that a pin PCNT reads needs its input buffer enabled, and that reconfiguring GPIO 6 output-only after `pcnt_new_channel` claimed it as a level input turns that buffer off — was tested on hardware and is wrong: `INPUT_OUTPUT` on GPIO 6 changed the count not at all across four runs, and adding it on GPIO 4 made it worse (0 of 10) by disturbing the routing `io_loop_back` sets up. Neither change was kept. What is fixed is everything around it: the unit no longer leaks, the count is read after the transmission rather than after a fixed delay, and the command now reports `#R:-1,failed` instead of claiming success while missing its own stated expectation. The remaining question is why the loopback delivers two edges, and it needs an instrument rather than another guess.
