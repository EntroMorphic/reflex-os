...
- **P0-M4 VM task self-delete lifetime gap — fixed 2026-09-11, host-proven, board validation partial.** A task cannot free the stack it is standing on, so `vTaskDelete(NULL)` defers teardown (to the idle task under FreeRTOS; to the next scheduling decision under Reflex's scheduler). The watchdog's stop-then-start therefore created the replacement while the previous stack was still allocated. `reflex_vm_task_stop` no longer waits on the handle and hopes: the entry publishes `finished`, parks when a stopper is waiting, and the stopper calls `reflex_task_delete(handle)` — which reclaims TCB and stack in the stopper's own context. Both targets create tasks with `xTaskCreate`, so FreeRTOS frees a non-current task's memory immediately; Reflex's scheduler marks it DEAD and `reflex_sched_reap` collects it at the next `pick_next`, the first moment execution is provably off that stack.
  ...
  **The correlation is with this change and it is not yet ruled out.** C6-B passed 183/0 on the previous build, was flashed with this one, and was deaf on its first boot; C6-A the same. Every passing C6 run this session predates this firmware. The V3 is unaffected, but it is the one target that does not use Reflex's own USB-serial-JTAG console RX. A mechanism exists: `reflex_vm_task_runtime_t` grew two `volatile bool`s and `system_vm` is a file-scope static, so `.bss` layout shifted on the one target carrying custom linker fragments and its own bootloader. Six environmental explanations were each tried and killed by evidence (boot-loop protection, a bad image, USB re-enumeration, hand-toggled DTR/RTS, the intervening `esptool chip_id`, a stale process holding the port). Recorded as **suspected regression, unproven**, under investigation; a power cycle has not yet been tried and would be the cheapest disconfirmation.
