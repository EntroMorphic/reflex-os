...
- **The eviction ring is written but never read.** `goose_fabric_get_eviction_ring` is declared in `goose.h` and has no caller anywhere in the tree — not the shell, not the Python SDK, not the viewer. Every eviction still pays an `snprintf` of the victim's name under `loom_authority` to fill it. The mechanism is sound and thrash detection is genuinely worth having, so this is left in place rather than deleted on my own initiative; it wants either a shell command that reads it or removal. Small — a few microseconds against a 637us peak — but it is the same "computes correctly, affects nothing" shape as the gateway that was removed on 2026-08-12.
- **Telemetry name buffers still truncate at 40 characters.** The deferred-telemetry and route-name buffers in `goose_supervisor.c` and `goose_runtime.c` are `char[40]`, so atlas names longer than 39 characters are cut short in the telemetry stream. Bounded and safe (`snprintf`), and display-only — the registry itself now stores the full name. Left at 40 deliberately: those buffers are stack-allocated inside the loom hold, and widening them to 96 would grow the stack footprint of the locked region.
