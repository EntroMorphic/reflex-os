...
- **Mesh health can never read +1 (connected)**: `REFLEX_METABOLIC_MESH_SPARSE_THRESHOLD` requires 5 arcs within one ~1.1s vital scan (~4.5 arcs/sec), while a quiet two-board mesh produces ~0.1 arcs/sec from the ~10.1s discovery beacon — roughly 46x short. `perception.mesh.health` is therefore effectively binary {0, -1}, and the "connected: relax discovery" branch in `goose_supervisor_pulse` (`state == 1` -> `discover_interval *= 2`) never fires. Fixing this is a calibration decision rather than a defect fix: the threshold would need to count arcs over a window instead of per-scan, or drop to 1 and accept the vital flickering between +1 and 0 between beacons. Left as-is pending that decision.
- **Shell coordinate display shows 3 of 9 trits**: `goonies find` prints `(trits[0], trits[3], trits[6])`, so cells in different coordinate namespaces render identically — `peer.alpha.led` at (5,0,0) and the atlas cell at (5,0,0) are distinct cells that display the same triple.
...
- **Shadow paging stops permanently once the Loom saturates**: diagnosed 2026-08-12, replacing an earlier and incorrect "coordinate collision" hypothesis — all 12738 atlas coordinates are in fact distinct (verified: 0 duplicates, and a 300-entry stride sample yields 300 distinct coordinates). The real cause is capacity. `goonies_registry[]` and `fabric_cells[]` are both `REFLEX_FABRIC_MAX_CELLS` (256). A fresh boot holds 114 named cells; `bonsai bloat` pages in exactly 142 more and stops at 256 on the nose. It cannot recover, because every shadow-paged cell is `HARDWARE_IN`/`HARDWARE_OUT` and the eviction loop in `goose_fabric_alloc_cell` skips both types — hence `evictions=0` while allocation fails. So the live Loom admits ~142 paged entries per boot out of 12738 and then refuses all further paging until reset.

  A contributing defect: `goose_fabric_alloc_cell` discards `goonies_register`'s return at both call sites (`goose_runtime.c:363`, `:400`), and that function returns `REFLEX_ERR_NO_MEM` once the registry hits 256. A cell allocated past the cap therefore occupies a coordinate with no registry entry — unreachable by name, and blocking that coordinate for the name that should own it. This is consistent with the observed instability where a second `bonsai bloat` pass over the same 300 names resolves fewer (142 then 101) rather than reusing the first pass's work; that delta is not yet fully accounted for.

  Name *resolution* via `goonies find` is unaffected — it falls through to `goose_shadow_resolve` without allocating. This bounds only how many catalog entries can be simultaneously *live*.
- **Supervisor sub-pass rates run ~9% slow**: the `if (div++ >= N)` idiom fires on the (N+1)-th pulse, so the documented 1Hz sub-passes run at ~0.909Hz. Uniform across all sub-passes and functionally harmless; recorded in `reflex_tuning.h` rather than changing every cadence at once.
- **Snapshots truncate past 16 routes**: the header records a field's full `route_count` but at most 16 entries are written. Load is bounded by blob length so it is safe to read, but plasticity beyond the 16th route of such a field is not persisted.
