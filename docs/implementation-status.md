...
- **Shadow paging resolves a fraction of sampled catalog names**: `bonsai bloat` striding 300 of the 12738 entries resolves ~102 deterministically, with `evictions=0` and spare Loom capacity (104 atlas cells + ~102 paged, cap 256). So this is neither saturation nor thrashing. The cause is not yet diagnosed — coordinate collisions in `goose_make_shadow_coord` are the leading hypothesis. Name *resolution* via `goonies find` is unaffected; this bounds how many catalog entries can be simultaneously *live*.
