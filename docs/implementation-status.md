...
- **ESP32 (Xtensa) uses a C6-derived MMIO atlas**: `goose_shadow_atlas.c` is generated from `tools/esp32c6.svd` but is compiled and used on every target. On the ESP32 board, `goonies find agency.gpio.bt_select` resolves and reports a C6 address that means something entirely different on that silicon. The atlas needs to be target-conditional (per-target SVD, or excluded on non-C6) before the ESP32 can be trusted for anything MMIO-related. Boot, services, mesh and the shell are unaffected.
- **ESP32 boot banner reports the wrong chip**: `chip=esp32-c6` is hardcoded in the boot log regardless of target.
...
- **Lock contention is observable on ESP32 but not on the C6**: `LOOM_CONTENTION_FAULT` appears on the dual-core ESP32, where `loom_authority` sees genuine parallelism for the first time. The single-core C6 has never produced one. Worth investigating before ESP32 is treated as more than a mesh peer.
- **`reflex_service_start_all` aborts every remaining service on the first failure**, and `main.c` then skips the atmospheric arcing block entirely. That is how an absent optional temperature sensor cost the ESP32 its whole radio substrate until 2026-08-12. The temp service no longer fails, but the fragility is structural: any optional service that fails still takes the mesh with it.

- **`goose_fabric_set_agency` rejects GPIO pin numbers**: the Sanctuary Guard masks its argument to a 4 KB page, so a pin number like 15 becomes page 0, which is not on the whitelist and is therefore treated as protected. `led_service` binds `led_phys` with `REFLEX_LED_PIN`, that call fails, and the cell keeps the `PINNED` type `alloc_cell` gave it instead of becoming `HARDWARE_OUT` — visible as `type=5` in `goonies find led_phys`. The LED still responds because the shell drives the GPIO directly, but the fabric route to it is decorative: its endpoints never leave state 0. That in turn starves Hebbian learning, which skips any route whose source and sink are both zero. The Guard needs to distinguish a GPIO pin index from an MMIO address rather than treating every small integer as page 0.
