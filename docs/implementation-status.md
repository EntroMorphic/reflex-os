...
- **The loom peak is a tail statistic, measured eleven times** (2026-09-11, both C6s). Reboot, three `bonsai bloat` passes, 761 evictions, identical firmware: C6-A `699 1006 694 702 1027 700`, C6-B `1002 970 649 1006 568`. Range **568-1027us** (1.8x), roughly bimodal — the ~700 runs set the peak in the first pass at ~9.8s, the ~1000 runs are overtaken by a later pass at ~10.6-11.2s. `avg_us` is **52 in every run**, so the average is deterministic and only the maximum moves. Eviction counts reproduce exactly (161/461/761), so the workload is deterministic and the spread is the measurement, not the load. The peak site reads `fabric_alloc_internal` in all eleven, confirming by measurement an attribution that five NULL-passing call sites had previously made indistinguishable.
...
Summary: the current counts live in the generated scope table at the top of
[independence-dependency-map.md](independence-dependency-map.md), which
`make independence-doc` regenerates from the measurement and fails on if it
drifts. They are deliberately not repeated here — this sentence used to say
"21 ESP-IDF includes remain" and "none of the 23" in the same breath, two
numbers that disagreed with each other and with the measurement, because a
summary written by hand is a summary nothing recomputes.

Tier A is clear. None of what remains is in the substrate — all sit in platform backends, the kernel's FreeRTOS shims, or the shell's peripheral drivers. The console is no longer among them: the C6 owns receive and transmit, hardware-validated at 171/171 across ten runs. Tier F (build system, startup, heap, linker/memory layout, image format) is untouched and is what actually decides independence: FreeRTOS cannot leave the image while newlib pulls in `esp_timer` and `pthread`, regardless of who schedules.
...
**The peak is a tail statistic, and the "2.4x discrepancy" was two single draws from a wide distribution** (measured 2026-09-11). The entry above compared one 1534us reading against one 637us reading as though a fixed quantity had changed. It is not fixed. Eleven runs of the identical workload — reboot, three `bonsai bloat` passes, 761 evictions, both C6s, same firmware — give:

    C6-A  699  1006  694  702  1027  700
    C6-B  1002  970  649  1006  568

Range **568–1027us**, a 1.8x spread with nothing varying but the run, and roughly bimodal (~650–710 and ~970–1030). The mode separates by *when* the peak lands: the ~700 runs set it during the first pass at ~9.8s and never exceed it, the ~1000 runs are overtaken by a later pass at ~10.6–11.2s. Meanwhile `avg_us` is **52 in all eleven runs** — the average cost is deterministic and only the maximum wanders, which is what a tail looks like.

So the honest statement is that comparing single peak readings across sessions was never valid, and "unexplained" was the wrong frame for a number that varies 1.8x by itself. What can be said: at saturation the peak hold is **2–3.4x `LOOM_LOCK_TIMEOUT_US`** (300us), the eviction scan runs inside the hold, and any concurrent `try_lock` in that window defers. Still not a correctness defect — no contention faults are logged and deferral is the designed response.

**Not fully explained:** 1534us remains above the 1027us observed maximum here, so the spread does not account for all of it. That reading was taken on a board that had run `hw-test` first; this sweep did not reproduce it in eleven attempts. It is recorded as an outlier rather than as a mystery about the current code.

**The site is confirmed, not inferred.** `goose_loom_try_lock` took a `goose_field_t *` and rendered a NULL field as the literal `"alloc"`, and **five** of the seven call sites pass NULL — `fabric_alloc_internal`, `goose_supervisor_weave_sync`, `goose_supervisor_learn_sync`, `goose_snapshot_save` and `goose_snapshot_load` all reported identically, so a peak labelled `alloc` named one of five sites, four of which allocate nothing. The label now carries `__func__` from the call site (proved to name the caller, not the callee, by a host probe). All eleven samples above, and the boot-time peak, report `fabric_alloc_internal` — so the original attribution was right, and is now evidence rather than assumption.
...
- **P0-M4 VM task self-delete lifetime gap — fixed 2026-09-11, host-proven, board validation partial.** A task cannot free the stack it is standing on, so `vTaskDelete(NULL)` defers teardown (to the idle task under FreeRTOS; to the next scheduling decision under Reflex's scheduler). The watchdog's stop-then-start therefore created the replacement while the previous stack was still allocated. `reflex_vm_task_stop` no longer waits on the handle and hopes: the entry publishes `finished`, parks when a stopper is waiting, and the stopper calls `reflex_task_delete(handle)` — which reclaims TCB and stack in the stopper's own context. Both targets create tasks with `xTaskCreate`, so FreeRTOS frees a non-current task's memory immediately; Reflex's scheduler marks it DEAD and `reflex_sched_reap` collects it at the next `pick_next`, the first moment execution is provably off that stack.

  Self-halt still self-deletes, deliberately: a program that halts or faults on its own has no stopper, and parking with no reaper would hold the stack for the rest of the boot. `REFLEX_VM_TASK_REAP_MS` survives only for the narrow race where the entry retired just before `stop()` claimed it.

  **Proven by mutation, not assertion.** Host suite 723/0 with a new `[vmreap]` test. Reverting to handle-observation drops it to **722/1** (`task was actually deleted, not merely forgotten`); removing the guard that refuses to delete a task which never published `finished` drops it to **720/3**. A third mutation — parking unconditionally — is **not caught**, and that is a real coverage gap rather than an equivalent mutant: it would leak a self-halting task's stack forever. The host mock never invokes task bodies, so the self-delete branch is unreachable there; a hook would hang the suite under that mutation rather than fail it.

  **Two honest limits.** The fix's hot path is unreachable in the shipped firmware: `reflex_vm_task_service_start` returns early when `runtime->image == NULL`, and `main.c` never assigns `system_vm.image`, so no VM task is ever created and the watchdog never restarts one. The shell offers no `vm task` verb either, so a board cannot be driven into the path on demand. Hardware therefore shows absence of regression, not exercise of the fix: **V3 177 passed / 0 failed / 4 skipped** on the final source. Both C6s boot cleanly on it (`console: cpu_int=10 plic_en=1`, `reflex>`, `system_stable=confirmed`, no panic) but **neither accepts console input on this firmware**: TX healthy, RX deaf, unchanged by reflash or reset.

  **Resolved: the firmware was not implicated, and all three boards validate.** The two C6s report **183 passed / 0 failed** on this firmware; the V3 **177 / 0 / 4**. What is established is the exoneration: the identical image passes, so nothing in the build explains it. The board was healthy throughout, printing a full boot log and `reflex>` while ignoring input, and it survived reflashing (the same image), resets and `esptool`. That the fault sits on the host side of the USB link is **inferred, not measured** — nothing here instruments the host, and a known-good earlier image was never flashed onto a deaf board, so that disconfirmation remains untried. Reopening the port cleared it, but the ingredient was not isolated: the first open after a reset returned buffered boot output and a second open answered, so a fresh open and a settle are confounded. `tests/hardware/validate_shell.py` opens once and polls for 25s and in every observed case did not recover a port already in that state; waking it first was enough. Six firmware and bench explanations were tried and killed by evidence before the host side was examined — the V3, on a CP210x bridge rather than native USB-serial-JTAG, was the control that should have pointed there sooner.
...
Open from the 2026-09-07 P0 audit: **P0-M4's proper fix is implemented and host-proven; its board validation is partial** (V3 only). See the entry above. Everything else is closed.
...
- ~~`vm run <program>` executes the VM synchronously in the shell task and makes the board unresponsive~~: **fixed 2026-09-11.** The step budget bounded work and not time, and a single step can block: `TSYS`'s log syscall writes to the console at about two lines a second, so 100,000 steps was a budget of roughly fourteen hours. `reflex_vm_run_bounded` adds a wall-clock bound, checked every step because the failure mode is one step that blocks for half a second, and the shell passes `REFLEX_VM_SHELL_RUN_BUDGET_MS` (2,000 ms). Measured on hardware: `vm run blink` returns in **2.6 s** where it previously needed a hardware reset, `status` answers immediately after, and `vm stop` — unreachable before because the shell was inside the run loop — works. The bound is "budget plus one step": the check follows the step that overran it, which is the 499 ms of overshoot observed. The VM task keeps the unbounded call, since it owns its own task. Note the interpreter's run loop has no host coverage — the host suite stubs `reflex_vm_run` — so this was verified on hardware only.
...
- ~~`internal_process_transitions`' `depth` parameter is vestigial~~: **removed 2026-09-11.** Nothing recursed into it, so `depth > 3` was unreachable and both `depth == 0` lock conditions were always true. Verified before removing that the loom is balanced on every path — there is no `return` between the acquire and the release. Code that appears to guard re-entry it does not have invites the reader to believe the re-entrant case was considered.
...
- **`make verify` advertised an independence ratchet it had not applied** (found and fixed 2026-09-11). `independence-check` runs `check_independence.py --check`, and with no usable `build_independence/` the tool printed "Upper bound only — the ratchet is not applied", counted 20 on-path against sources that configuration never compiles, and exited 0 — while verify's banner read "the independence ratchet in both configurations ... all passed". `independence-own-entry` could not fail this way because it depends on `own-entry-build`; the asymmetry is what hid it, and a fresh clone (which has never built that tree) got the same false assurance by default. `--check` now exits non-zero when the ratchet cannot be applied; the advisory mode still reports an upper bound and succeeds. The gate keys on a real compile database, not on the directory existing — verified against a half-built tree, which it correctly refused. Proven in both directions: with the tree complete it exits 0, applies the ratchet and reports the real measurement (**8 on-path**), where unmeasurable it reported a 20-file upper bound. A gate that cannot pass would have been no better than one that cannot fail.
...
- ~~`bonsai exp5` counts 2 of the 10 pulses it sends~~: **fixed 2026-09-08**, by owning RMT. The pad's input buffer was never enabled, so the counter could not see the signal the transmitter was driving — `io_loop_back`, the ESP-IDF flag meant to arrange that, is deprecated in this version. Both peripherals were configured identically to ESP-IDF's own registers throughout, which is precisely why nothing in their configuration explained it. Isolated by removing one `IO_MUX_FUN_IE` write and changing nothing else: 10 of 10 becomes 0 of 10 with every register unchanged. The three guesses tried before were all aimed at a pin that was already correct.
...
- ~~The ESP32 fabric does not survive deep sleep~~ — the *comment* half of this is **already fixed** and the entry was stale: `include/reflex_hal.h` now states plainly that the classic ESP32 is empty because the data does not fit (12,288 bytes of `fabric_cells[]` against 8,168 usable) and explicitly retracts the earlier claim that it survives deep sleep. The behaviour is unchanged and benign — `fabric_magic` reads zero after any reset, so the cold-boot branch always runs.
