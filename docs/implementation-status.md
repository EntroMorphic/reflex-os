...
- **The loom's worst holder is `goose_fabric_get_cell_by_coord`'s linear fallback, and the lattice index cannot fix it.** `lattice_index` stores one slot per bucket with no collision handling (`goose_runtime.c:495` and the rebuild at `:304` both just overwrite), so with 256 cells hashed into 503 buckets roughly a fifth of cells are permanently shadowed and reachable only by the 256-entry linear scan — about 95us per call at nine trit comparisons each. Allocation always pays it (a new coord cannot hit the index), and pays it twice more via the victim reverse-lookup and `goonies_register`'s 256-entry `strcmp` scan. That is the ~600us peak. Shortening it properly means adding collision resolution (open addressing or chaining), which is a real change to the fabric's core lookup and wants its own pass.
