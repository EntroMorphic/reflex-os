...
| `kernel wdt [ms\|off\|observe <ms>\|expired\|entry\|why]` | Read, disarm, or arm the low-power watchdog (admin). Arming is behind a build flag: a fired watchdog does not return a usable board. `observe` arms it to latch rather than reset, so it is safe to run; `expired` reads that latch; `entry` reports the state this boot started with, captured before startup disarmed it; `why` reports what caused the last reset (power-on, deep-sleep wake, a watchdog) |
